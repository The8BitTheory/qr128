...
# F:START OF MATRIX
# G:END OF MATRIX
6226 O=F:C=0:R=0:GRAPHIC0:PRINT"":PRINT " ";
6228 DO WHILE R<(L-1)/2

6230  V1=PEEK(O)AND1:V2=PEEK(O+1)AND1:V3=PEEK(O+L)AND1:V4=PEEK(O+1+L)AND1
6240  IF C<L-1 THEN V=V1+V2*2+V3*4+V4*8:ELSE V=V1+V3*4
6245  IF R>=L-1 THEN IF C>=L-1 THEN V=V1
6250  PRINT CH$(V);
6260  O=O+2
...
6272 V1=PEEK(O)AND1:V2=PEEK(O+1)AND1
6273 IF C<L-1 THEN V=V1+V2*2:ELSE V=V1
6275 PRINT CH$(V);
6277 O=O+2:C=C+2
6278 NEXT
6279 PRINT " "
...
# SET REG 23 TO 5
# SET REG 9 TO 4
# SET REG 4 TO 320/5=63
6300 VW=DEC("CDCC"):BANK15
6301 SYSVW,5,23
6302 SYSVW,4,9
6303 SYSVW,63,4
6304 SYSVW,37,6
...
6326 O=F:C=0:R=0:GRAPHIC5:PRINT"";
6328 DO WHILE O<G

6340  V=PEEK(O)AND1
6350  IF V=1 THEN PRINT CHR$(18)" ";:ELSE PRINT " ";
6360  O=O+1
6365  C=C+1:IF C >= L THEN C=0:PRINT
6370 LOOP
6380 PRINT:GRAPHIC5
