...
# DIVISORS
30 E=D+26:F=E+26:PRINT "MATRIX AT "F
...
1098 PRINT "PRESS KEY TO SHOW QR-CODE"
1099 GET GK$:IF GK$="" THEN 1099
1100 GOTO 6000
...
6229  PO=PEEK(O):V1=POAND1
6230  IF ((PO AND 64)=64) THEN YY=V1:V1=FNXR(ABS((PO AND FT)=FT))

6231  PO=PEEK(O+1):V2=POAND1
6232  IF ((PO AND 64)=64) THEN YY=V2:V2=FNXR(ABS((PO AND FT)=FT))

6233  PO=PEEK(O+L):V3=POAND1
6234  IF ((PO AND 64)=64) THEN YY=V3:V3=FNXR(ABS((PO AND FT)=FT))

6235  PO=PEEK(O+1+L):V4=POAND1
6236  IF ((PO AND 64)=64) THEN YY=V4:V4=FNXR(ABS((PO AND FT)=FT))

6240  IF C<L-1 THEN V=V1+V2*2+V3*4+V4*8:GOTO 6250
6241  V=V1+V3*4
...
6273  PO=PEEK(O):V1=POAND1
6274  IF ((PO AND 64)=64) THEN YY=V1:V1=FNXR(ABS((PO AND FT)=FT))

6275  PO=PEEK(O+1):V2=POAND1
6276  IF ((PO AND 64)=64) THEN YY=V2:V2=FNXR(ABS((PO AND FT)=FT))

6277  IF C<L-1 THEN V=V1+V2*2:GOTO 6279
6278  V=V1
6279  PRINT CH$(V);
6281  O=O+2:C=C+2
6283 NEXT
...
# 
7000 S(7)=TI:S$(7)="CALCULATE AND APPLY XOR MASKS"
7001 O=F
7005 FOR Y=0 TO L-1
7010  FOR X=0 TO L-1
...
11020  C=INT(R/128)
11030  IF C=1 THEN C=285:GOTO 11040
11031  C=0
#C=C*285
11040  YY=C:R=FNXR(B)
#:PRINT " R"R
